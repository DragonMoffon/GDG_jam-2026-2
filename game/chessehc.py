...
class Board:
    ...
    def __getitem__(self, location: tuple[int, int]) -> Tile:
        ...
        elif self.mirror_slice <= 0 and self.width + self.mirror_slice <= x:
    ...
    def location_mirrored(self, location: tuple[int, int]):
        x, _ = location
        if self.mirror_slice is None:
            return False

        return bool(
            (0 <= self.mirror_slice and x < self.mirror_slice)
            or (self.mirror_slice <= 0 and self.width + self.mirror_slice <= x)
        )
        ...
        with self.camera.activate():
            ...
            for x in range(self.board.width):
                for y in range(self.board.height):
                    if self.board.location_mirrored((x, y)):
                        color = (0, 0, 255)
                    else:
